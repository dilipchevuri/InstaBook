  ...
  return (
    ...
    >
      <div className="font-mono text-xs text-inkmuted reading-muted uppercase tracking-widest mb-2">
        {new Date(note.date).toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}
      </div>
      <div className="font-display text-xl text-ink reading-heading font-semibold">{note.title}</div>
    </Link>
  );
}
